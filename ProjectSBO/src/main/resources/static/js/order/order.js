	...
	function (rsp) { // callback
		if (rsp.success) {
         // 결제 성공 시 로직,
	        ...
	        // 차례대로 승인번호, 카드이름, 카드번호, 승인금액, 결제 일자(년월시)
	        data.APPROVAL_NO = rsp.apply_num;
	        data.CARD_NAME = rsp.card_name;
	        data.CARD_NUMBER = rsp.card_number;
	        data.APPROVAL_AMOUNT = rsp.paid_amount;
	        var aa = Unix_timestamp(rsp.paid_at);
	        data.paid_at = aa;
...
function paymentCash(data){
	//원큐페이시
	//window.open("http://192.168.0.3:8080/pay");
	$.ajax({
